...
\begin{itemize}
  ...
  \item Hiperparâmetros otimizados via \textbf{Grid Search exaustivo + 5-fold CV}
        estratificado (\texttt{GridSearchCV}, scikit-learn) para todos os 18 modelos
        no Tier 1 (Transformers re-executados com \texttt{epochs=40}, \texttt{patience=6}
        em GPU T4). Todos os modelos Tier 2 utilizam Optuna TPE (20 trials, 3-fold CV).
...
\begin{itemize}
  \item \textbf{Nyström-SVM} ($0{,}838$) é o modelo com melhor F1-macro médio,
        ...
        --- os três empatam estatisticamente. Com $\approx 78\%$ de esparsidade
  ...
  \item \textbf{DualFISTA} ($0{,}837$) alcança F1-macro médio essencialmente
        ...
        $\approx 5\%$ de esparsidade. É o único modelo esparso que se aproxima
...
Resultado: $\chi^2 = 55{,}48$, $p \approx 0$, $\text{CD} = 8{,}65$
...
\midrule
1  & LSSVM-Nyström       & 3,000 & 0,776 \\
2  & LSSVM-Std           & 3,333 & 0,775 \\
3  & LSSVM-DualFISTA     & 3,833 & 0,773 \\
4  & XGBoost             & 5,833 & 0,766 \\
5  & LSSVM-IP            & 6,167 & 0,766 \\
6  & LSSVM-FSA           & 7,833 & 0,766 \\
\midrule
7  & \textbf{FT-Entmax}  & 9,000 & 0,757 \\
8  & LSSVM-PCP           & 9,167 & 0,763 \\
9  & \textbf{FT-CUR}     & 9,333 & 0,759 \\
10 & \textbf{FT-Sparsemax} & 10,167 & 0,752 \\
11 & \textbf{SAINT}      & 10,333 & 0,755 \\
12 & \textbf{FT-Softmax} & 10,667 & 0,755 \\
13 & LSSVM-Pruning       & 11,833 & 0,733 \\
14 & LSSVM-ADMM          & 12,917 & 0,734 \\
15 & \textbf{FT-TopK}    & 13,500 & 0,736 \\
16 & LSSVM-ADMM-EN       & 13,583 & 0,735 \\
17 & LSSVM-FISTA         & 13,667 & 0,732 \\
...
significativamente no ranking. FT-Entmax (rank 9,0) e FT-CUR (rank 9,3)
superam LSSVM-PCP, LSSVM-Pruning, LSSVM-ADMM e LSSVM-FISTA. A diferença
entre o bloco líder LSSVM (ranks 3,0--3,8) e os melhores Transformers
(rank 9,0--9,3) é inferior ao CD ($8{,}65$), portanto
...
ADMM-ElasticNet$^\circ$
  ...
  & Similar ao paper-base, ligeiramente mais homogênea. Média: $\approx 7\%$ (Tier 1, Grid+CV). \\
\addlinespace
DualFISTA$^\circ$
  & Formulação dual:
    $\min_\alpha \frac{1}{2}\alpha^\top\Omega\alpha - \mathbf{1}^\top\alpha + \lambda\|\alpha\|_1$,
    otimizado via FISTA. Não localizada na literatura nesta forma exata.
  & Mais uniforme que ADMM (dual bem condicionado). Média: $\approx 5\%$ (Tier 1, Grid+CV). \\
\addlinespace
Nyström-SVM$^\star$
...
FT-CUR (Nyströmformer)$^\star$ & 0,738 & --- & $85{,}2\%$ & 0,849 & $+$0,111 & 0,677 & $-$0,061 & 0,809 \\
SAINT (baseline FT)  & 0,731 & --- & $0\%$ & 0,862 & $+$0,131 & 0,672 & $-$0,059 & 0,807 \\
\bottomrule
\end{tabular}}
\end{table}
...
DualFISTA$^\circ$
  ...
  & Sensível a features de ruído (kernel RBF); esparsidade baixa ($\approx 5\%$ no Tier 1)
...
\begin{itemize}
  ...
  \item \textbf{DualFISTA} fica em $2{,}1\%$ de esparsidade no Tier 2 ---
        muito abaixo dos $5\%$ do Tier 1 (Grid+CV). O Optuna passou a preferir
...
\begin{enumerate}
  ...
  \item \textbf{Transformers e XGBoost.} FT-Transformers ($^\dagger$,
        protocolo Optuna no Tier 2) absorvem a transição com baixa perda
...
Ablação A (escala) & 11 mod.\ LSSVM+XGB (em andamento), 3 sintéticos, $N{=}2000$, 30s & 649+ & ADMM $+0{,}072$ em TWC; Transf.\ pendentes (Kaggle) \\
Ablação B (ruído)  & 12 mod.\ LSSVM+XGB, 3 sintéticos, $+$3 feat.\ ruído, 30s & 1080 & XGBoost $\overline{\Delta}{=}{-}0{,}048$; LSSVMs: $-0{,}26$ a $-0{,}39$; Transf.\ pendentes \\
Ablação C (MK5)    & 12 mod.\ LSSVM+XGB, 3 sintéticos, 30 seeds         & 1080 & LSSVM-Std 0,851 $>$ Nyström 0,841 $>$ XGBoost 0,827; Transf.\ pendentes \\
\midrule
