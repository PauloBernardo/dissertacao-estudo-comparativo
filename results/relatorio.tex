...
\emph{todos} os modelos, usando Grid Search exaustivo + 5-fold CV para LSSVMs e
XGBoost. Os melhores valores por coluna estão em negrito. Resultados de
Transformers são do protocolo Optuna anterior ($\dagger$), pendentes de
re-execução com Grid+CV em GPU.

\begin{table}[H]
\centering
\caption{F1-macro médio (30 sementes). \textbf{Negrito}: melhor por coluna.
$\circ$ = variante investigada; $\star$ = proposta.
$\dagger$ = protocolo Optuna (Grid+CV pendente para Transformers).}
...
\midrule
\multicolumn{11}{l}{\textit{Baselines FT-Transformer ($\dagger$ protocolo Optuna; Grid+CV pendente)}} \\
FT-Sparsemax$^\dagger$    & 0,950 & 0,691 & 0,596 & 0,808 & 0,662 & 0,855 & 0,690 & 1,000 & 0,710 & 0,774 \\
SAINT$^\dagger$           & 0,949 & 0,710 & 0,535 & 0,774 & 0,676 & 0,859 & 0,639 & 0,969 & 0,588 & 0,744 \\
FT-Entmax$^\dagger$       & 0,953 & 0,700 & 0,538 & 0,800 & 0,657 & 0,860 & 0,667 & 0,999 & 0,504 & 0,742 \\
FT-Softmax$^\dagger$      & 0,959 & 0,708 & 0,541 & 0,810 & 0,658 & 0,854 & 0,653 & 1,000 & 0,478 & 0,740 \\
FT-TopK$^\dagger$         & 0,950 & 0,692 & 0,501 & 0,808 & 0,669 & 0,857 & 0,640 & 0,974 & 0,468 & 0,729 \\
\midrule
...
FT-CUR$^{\star\dagger}$ (Nyströmformer) & 0,946 & 0,715 & 0,523 & 0,824 & 0,665 & 0,851 & 0,654 & 0,981 & 0,564 & 0,747 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{itemize}
  ...
  \item Resultados de FT-Transformers ($\dagger$) são do protocolo Optuna
        anterior e estão pendentes de re-execução com Grid+CV em GPU.
  \item \textbf{SAINT} ($0{,}744$) e \textbf{FT-CUR} ($0{,}747$) atingem F1
        essencialmente idêntico (diferença $0{,}003$). Como o FT-CUR comprime
        $\approx 79\%$ da atenção inter-instâncias e o SAINT é denso, isso
        valida a aproximação Nyströmformer: \textbf{a compressão não custa F1}.
\end{itemize}

\subsection{Análise estatística: Friedman e ranks médios (Tier 1)}
\label{sec:tier1_friedman}

O teste não-paramétrico de Friedman aplicado às 12 variantes LSSVM e XGBoost
sobre os 9 datasets confirma diferença estatisticamente significativa entre os
modelos: $\chi^2 = 48{,}82$, $p = 10^{-6}$. A diferença crítica de Nemenyi
para $\alpha = 0{,}05$ é $\text{CD} = 5{,}56$.

\input{tables/ranks.tex}

\textbf{Leitura dos ranks:} Nyström-SVM (rank 3,0), DualFISTA (3,2) e
LSSVM-Std (3,2) formam o bloco líder, estatisticamente equivalentes entre si
(gap $< \text{CD}$). XGBoost (rank 6,0) posiciona-se como o melhor modelo
não-kernel: superior a FSA (6,1) e PCP (6,6). FISTA-Nesterov (10,0) e
OppositeMaps (9,6) ficam no extremo inferior, consistentemente com seus
colapsos em HAB e PID.
