...
\begin{itemize}
  ...
  \item Hiperparâmetros otimizados via \textbf{Grid Search exaustivo + 5-fold CV}
        estratificado (\texttt{GridSearchCV}, scikit-learn) para modelos LSSVM e
        XGBoost no Tier 1. Transformers Tier 1 (pendente re-execução em GPU) e
        todos os modelos Tier 2 utilizam Optuna TPE (20 trials, 3-fold CV).
...
LSSVM-PCP       & 0,916 & 0,717 & \textbf{0,627} & 0,783 & 0,684 & 0,849 & 0,990 & 0,993 & 0,880 & 0,827 \\
FISTA-Nesterov  & 0,894 & 0,693 & 0,603 & 0,754 & 0,606 & 0,841 & 0,875 & 0,996 & 0,989 & 0,806 \\
...
FT-Sparsemax      & 0,945 & 0,675 & 0,557 & 0,800 & 0,655 & 0,857 & 0,629 & 0,895 & 0,469 & 0,720 \\
SAINT             & 0,945 & 0,709 & 0,530 & 0,813 & 0,598 & \textbf{0,863} & 0,632 & 0,833 & 0,446 & 0,708 \\
FT-Entmax         & 0,946 & 0,681 & 0,553 & 0,803 & 0,651 & 0,860 & 0,625 & 0,851 & 0,462 & 0,715 \\
FT-Softmax        & 0,953 & 0,681 & 0,560 & 0,812 & 0,664 & 0,852 & 0,622 & 0,847 & 0,456 & 0,716 \\
FT-TopK           & 0,948 & 0,649 & 0,536 & 0,768 & 0,644 & 0,856 & 0,635 & 0,826 & 0,479 & 0,704 \\
\midrule
\multicolumn{11}{l}{\textit{Paper-base estendido}} \\
LSSVM-ADMM-Nesterov     & 0,901 & 0,690 & 0,620 & 0,762 & 0,592 & 0,842 & 0,974 & 0,988 & 0,883 & 0,806 \\
\midrule
\multicolumn{11}{l}{\textit{Variantes investigadas / propostos}} \\
DualFISTA$^\circ$       & \textbf{0,976} & 0,730 & 0,558 & 0,824 & 0,687 & \textbf{0,863} & 0,992 & \textbf{1,000} & 0,905 & 0,837 \\
Nyström-SVM$^\star$     & 0,970 & 0,728 & 0,579 & 0,825 & \textbf{0,691} & \textbf{0,863} & 0,992 & 0,999 & 0,898 & \textbf{0,838} \\
ADMM-ElasticNet$^\circ$ & 0,901 & 0,689 & 0,620 & 0,766 & 0,591 & 0,841 & 0,974 & 0,988 & 0,879 & 0,805 \\
FT-CUR$^{\star}$ (Nyströmformer) & 0,952 & 0,697 & 0,565 & 0,804 & 0,627 & 0,859 & 0,622 & 0,864 & 0,455 & 0,716 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{itemize}
  \item \textbf{Nyström-SVM} ($0{,}838$) é o modelo com melhor F1-macro médio,
        seguido muito de perto por LSSVM-Std ($0{,}838$) e DualFISTA ($0{,}837$)
        --- os três empatam estatisticamente. Com $\approx 69\%$ de esparsidade
        amostral, o Nyström-SVM valida o trade-off F1/esparsidade como o melhor
        entre modelos de alta compressão.
  \item \textbf{DualFISTA} ($0{,}837$) alcança F1-macro médio essencialmente
        igual ao LSSVM-Std completo ($0{,}838$, diferença $0{,}001$) com
        $\approx 20\%$ de esparsidade. É o único modelo esparso que se aproxima
        do baseline sem esparsidade.
  \item \textbf{XGBoost} (baseline tabular) atinge $0{,}826$ no Tier 1 ---
        agora competitivo com os LSSVMs esparsos (IP $0{,}832$, PCP $0{,}827$).
  ...
  \item O \textbf{LSSVM-ADMM} e variantes (ElasticNet, FISTA-Nesterov)
        ficam $\approx 3$ pp abaixo do LSSVM-Std. FISTA-Nesterov ($0{,}806$)
        empata praticamente com ADMM-N ($0{,}806$) e ADMM-EN ($0{,}805$)
        após correção do grid de $\sigma$ (adição de $0{,}15$ e $1{,}5$,
        cobrindo a escala ótima para datasets sintéticos).
  ...
  \item \textbf{Transformers} (Grid+CV, GPU T4) ficam em $0{,}704$--$0{,}720$
        de F1 médio --- $12$--$13$ pp abaixo do bloco líder LSSVM. O colapso
        concentra-se nos datasets sintéticos: TWC melhor transformer ($0{,}479$,
        FT-TopK) contra Nyström-SVM $0{,}898$; TWS $0{,}635$ vs $0{,}994$.
        Nos datasets reais (BCW, AUS, VCP) os Transformers são competitivos,
        com SAINT empatando em AUS ($0{,}863$) com DualFISTA e Nyström-SVM.
  \item \textbf{SAINT} ($0{,}708$) e \textbf{FT-CUR} ($0{,}716$) ficam próximos
        no Grid+CV. FT-CUR comprime $\approx 79\%$ da atenção inter-instâncias
        mantendo F1 equivalente ao SAINT denso:
...
significativa entre os modelos: $\chi^2 = 69{,}71$, $p \approx 0$. A diferença
crítica de Nemenyi para $\alpha = 0{,}05$ é $\text{CD} = 8{,}78$.

\input{tables/ranks.tex}

\textbf{Leitura dos ranks:} Nyström-SVM (rank 3,1), LSSVM-Std (3,3) e
DualFISTA (3,4) formam o bloco líder, estatisticamente equivalentes entre si
(gap $< \text{CD}$). XGBoost (rank 6,9) é o melhor modelo não-kernel e
não-Transformer. Todos os Transformers ficam na metade inferior do ranking
(ranks 11,3--13,4): FT-CUR é o melhor deles (rank 11,3), seguido de
FT-Softmax (11,8) e SAINT (12,2). FT-TopK (13,4) e LSSVM-OppMaps (13,3)
são os piores. O colapso dos Transformers nos datasets sintéticos TWS/TWM/TWC
--- onde a fronteira de decisão exige inductive bias geométrico que a atenção
não possui --- é o principal responsável pelos ranks baixos.
...
Resultado: $\chi^2 = 88{,}38$, $p \approx 0$, $\text{CD} = 9{,}56$
(18 modelos $\times$ 6 datasets).
...
\midrule
1  & LSSVM-Nyström       & 3,167 & 0,776 \\
2  & LSSVM-Std           & 3,333 & 0,775 \\
3  & LSSVM-DualFISTA     & 3,833 & 0,773 \\
4  & XGBoost             & 6,167 & 0,766 \\
5  & LSSVM-IP            & 6,500 & 0,766 \\
6  & LSSVM-FSA           & 7,667 & 0,766 \\
\midrule
7  & \textbf{FT-CUR}     & 9,000 & 0,751 \\
8  & LSSVM-PCP           & 9,167 & 0,763 \\
9  & \textbf{FT-Softmax} & 9,333 & 0,754 \\
10 & \textbf{SAINT}      & 10,167 & 0,743 \\
11 & \textbf{FT-Entmax}  & 10,833 & 0,749 \\
12 & LSSVM-Pruning       & 11,500 & 0,733 \\
13 & \textbf{FT-Sparsemax} & 11,500 & 0,748 \\
14 & LSSVM-ADMM          & 12,750 & 0,734 \\
15 & \textbf{FT-TopK}    & 12,833 & 0,733 \\
16 & LSSVM-FISTA         & 13,167 & 0,732 \\
17 & LSSVM-ADMM-EN       & 13,417 & 0,735 \\
18 & LSSVM-OppMaps       & 16,667 & 0,658 \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Interpretação:} sem os datasets sintéticos, os Transformers sobem
significativamente no ranking. FT-CUR (rank 7) e FT-Softmax (rank 9)
superam LSSVM-PCP, LSSVM-Pruning, LSSVM-ADMM e LSSVM-FISTA. A diferença
entre o bloco líder LSSVM (ranks 3,2--3,8) e os melhores Transformers
(rank 9,0--9,3) é inferior ao CD ($9{,}56$), portanto
...
DualFISTA$^\star$ & 0,837 &  5,0\% & $\ell_1$ dual via FISTA \\
LSSVM-IP         & 0,832 & 46,1\% & iterative pruning \\
LSSVM-FSA        & 0,829 & 54,6\% & seleção forward \\
LSSVM-PCP        & 0,827 &  0,0\% & cone projection \\
LSSVM-Prun       & 0,808 & 62,9\% & pruning iterativo c/ \emph{early stop} \\
LSSVM-ADMM$^\star$& 0,806 &  7,9\% & $\ell_1$ primal (ADMM-Nesterov) \\
ADMM-ElasticNet$^\star$& 0,805 & 7,0\% & $\ell_1 + \ell_2$ (Elastic Net) \\
FISTA-Nesterov$^\star$ & 0,806 & 7,5\% & $\ell_1$ primal via FISTA \\
...
\begin{itemize}
  ...
  \item \textbf{DualFISTA} atinge apenas $5\%$ de esparsidade (era $20\%$ com
        Optuna) porque o Grid+CV seleciona $\lambda$ menores que maximizam F1 com
        pouca regularização. A perda em F1 é $0{,}001$ — negligível.
  \item Os métodos ADMM/FISTA (primal) apresentam esparsidade $7$--$8\%$ no
        novo protocolo (era $27$--$30\%$): o grid correto puxou $\lambda$
        para baixo. A queda em F1 em relação ao LSSVM-Std é $\approx 3$ pp.
...
\midrule
DualFISTA$^\circ$    & 0,837 &  5\% & ---       & 0,979 & $+$0,018 & 0,723 & $-$0,239 & 0,857 \\
LSSVM-ADMM (paper-base) & 0,806 &  8\% & ---  & 0,977 & $+$0,023 & 0,700 & $-$0,253 & 0,832 \\
ADMM-ElasticNet$^\circ$ & 0,805 &  7\% & ---  & --- & --- & --- & --- & --- \\
Nyström-SVM$^\star$  & \textbf{0,838} & \textbf{78\%} & --- & \textbf{0,978} & $+$0,144 & 0,685 & $-$0,149 & 0,845 \\
...
\midrule
LSSVM-ADMM (paper-base)  & 0,806 & 0,580 & $-$0,226 & 75,2\% \\
ADMM-ElasticNet$^\circ$ & 0,805 & 0,576 & $-$0,229 & 73,7\% \\
FISTA-Nesterov           & 0,806 & 0,577 & $-$0,229 & 49,1\% \\
\midrule
...
DualFISTA$^\circ$    & 0,837 & 0,706 & $-$0,131 & dual $\ell_1$ via FISTA \\
Nyström-LSSVM$^\star$ & 0,838 & 0,707 & $-$0,131 & kernel esparso (81\% esp.) \\
LSSVM-IP             & 0,832 & 0,694 & $-$0,138 & iterative pruning \\
LSSVM-PCP            & 0,827 & 0,685 & $-$0,142 & cone projection \\
LSSVM-FSA            & 0,829 & 0,679 & $-$0,150 & forward selection \\
\midrule
XGBoost              & 0,826 & 0,735 & $-$0,091 & gradient boosting \\
FT-Softmax$^\dagger$ & 0,740 & 0,725 & $-$0,015 & atenção feature densa \\
FT-Sparsemax$^\dagger$ & 0,774 & 0,723 & $-$0,051 & atenção feature esparsa \\
FT-Entmax$^\dagger$  & 0,742 & 0,723 & $-$0,019 & atenção feature esparsa \\
FT-TopK$^\dagger$    & 0,729 & 0,720 & $-$0,009 & atenção feature top-$k$ \\
SAINT$^\dagger$      & 0,744 & 0,722 & $-$0,022 & atenção inter-inst.\ densa \\
FT-CUR$^{\star\dagger}$ & 0,747 & 0,713 & $-$0,034 & atenção inter-inst.\ comprimida \\
\midrule
LSSVM-ADMM (paper-base)  & 0,806 & 0,580 & $-$0,226 & primal $\ell_1$ (ADMM) \\
ADMM-ElasticNet$^\circ$ & 0,805 & 0,576 & $-$0,229 & primal $\ell_1+\ell_2$ \\
FISTA-Nesterov            & 0,806 & 0,577 & $-$0,229 & primal $\ell_1$ (FISTA) \\
...
\begin{enumerate}
  \item \textbf{LSSVMs duais bem-comportados ($\Delta \approx -0{,}13$ a $-0{,}15$).}
        ...
        empatados com LSSVM-Std denso} (gap $< 0{,}002$), confirmando que os
  ...
  \item \textbf{Transformers e XGBoost.} FT-Transformers ($^\dagger$,
        protocolo Optuna) absorvem a transição com baixa perda
        ($\Delta \approx -0{,}01$ a $-0{,}05$). As variantes esparsas convergem
        para o mesmo patamar do softmax denso ($0{,}720$--$0{,}725$): a
        esparsidade na atenção inter-features não traz vantagem mensurável.
...
\textbf{Reordenação:} no Tier 1, o ranking é Nyström/LSSVM-Std/DualFISTA
(empatados em $\approx 0{,}838$) $>$ IP/FSA/PCP/XGBoost ($0{,}826$--$0{,}832$)
...
\midrule
Tier 1            & 12 mod.\ LSSVM+XGB, 9 datasets, 30 seeds (Grid+CV) & 3240 & Nyström 0,838 / LSSVM-Std 0,838 / DualFISTA 0,837 / XGBoost 0,826 \\
...
\begin{enumerate}
  ...
  \item \textbf{Colapso dos métodos primais com $\ell_1$ em escala} é o
        ...
        FISTA-Nesterov caem de $\approx 0{,}806$ (Tier 1, Grid+CV) para
        $\approx 0{,}58$ em $N=5000$. Revela um teto
