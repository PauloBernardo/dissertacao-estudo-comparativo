...
\begin{itemize}
  ...
  \item \textbf{FT-CUR} ($0{,}738$) é o melhor Transformer, empatado em rank
        com FT-Entmax ($0{,}734$). FT-CUR comprime $\approx 79\%$ da atenção
...
significativa entre os modelos: $\chi^2 = 72{,}84$, $p \approx 0$. A diferença
crítica de Nemenyi para $\alpha = 0{,}05$ é $\text{CD} = 8{,}78$.

\input{tables/ranks.tex}

\textbf{Leitura dos ranks:} Nyström-SVM (rank 3,0), LSSVM-Std (3,3) e
DualFISTA (3,4) formam o bloco líder, estatisticamente equivalentes entre si
(gap $< \text{CD}$). XGBoost (rank 6,7) é o melhor modelo não-kernel e
não-Transformer. Todos os Transformers ficam na metade inferior do ranking
(ranks 11,1--14,3): FT-CUR e FT-Entmax empatam no melhor posto entre
Transformers (rank 11,1), seguidos de FT-Sparsemax (11,7) e LSSVM-ADMM (11,8).
FT-TopK (14,3) é o pior Transformer. O colapso dos Transformers nos datasets
sintéticos TWS/TWM/TWC --- onde a fronteira de decisão exige inductive bias
geométrico que a atenção não possui --- é o principal responsável pelos ranks
baixos.
