...
Reads results/tier2_n5000_combined.json and outputs:
  results/tables/  — LaTeX .tex files
  results/plots/   — PDF figures

Usage
-----
    python scripts/generate_analysis.py [--results results/tier2_n5000_combined.json]
                                        [--metric f1_macro]
"""
...
MODEL_LABELS: dict[str, str] = {
    ...
    "NystromLSSVMColnorm":      "Nyström-LSSVM",
...
def run(results_path: Path, metric: str) -> None:
    ...
    tex = results_table(
        ...
        metric=metric,
        caption=f"Média $\\pm$ desvio padrão de F1-macro (30 sementes, 6 conjuntos de dados Tier 2 (N=5000)).",
        label="tab:tier2_results",
    )
    (TABLES_DIR / "tier2_results.tex").write_text(tex)
    print(f"  Saved tables/tier2_results.tex")
    ...
    (TABLES_DIR / "tier2_accuracy.tex").write_text(tex_acc)
    print(f"  Saved tables/tier2_accuracy.tex")
    ...
    (TABLES_DIR / "tier2_sparsity.tex").write_text(tex_sp)
    print(f"  Saved tables/tier2_sparsity.tex")
    ...
    (TABLES_DIR / "tier2_friedman.txt").write_text(
    ...
    (TABLES_DIR / "tier2_ranks.tex").write_text(tex_ranks)
    print(f"  Saved tables/tier2_ranks.tex")
    ...
    (TABLES_DIR / "tier2_wilcoxon_raw.tex").write_text(tex_wilcoxon_raw)
    print(f"  Saved tables/tier2_wilcoxon_raw.tex")
    (TABLES_DIR / "tier2_wilcoxon.tex").write_text(tex_wilcoxon)
    print(f"  Saved tables/tier2_wilcoxon.tex")

    # ── 5. CD diagram ─────────────────────────────────────────────────────────
    print("Generating CD diagram ...")
    fig = cd_diagram(ranks, labels_v, cd,
                     title=f"Diagrama de Diferença Crítica — {metric}")
    fig.savefig(PLOTS_DIR / "tier2_cd_diagram.pdf", bbox_inches="tight")
    fig.savefig(PLOTS_DIR / "tier2_cd_diagram.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print(f"  Saved plots/tier2_cd_diagram.pdf")

    # ── 6. Box plots ──────────────────────────────────────────────────────────
    print("Generating box plots ...")
    fig = boxplots(df_display.to_dict("records"), metric=metric,
                   title=f"Distribuição de {metric} por dataset")
    fig.savefig(PLOTS_DIR / "tier2_boxplots.pdf", bbox_inches="tight")
    fig.savefig(PLOTS_DIR / "tier2_boxplots.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print(f"  Saved plots/tier2_boxplots.pdf")
    ...
    fig.savefig(PLOTS_DIR / "tier2_sparsity_scatter.pdf", bbox_inches="tight")
    fig.savefig(PLOTS_DIR / "tier2_sparsity_scatter.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print(f"  Saved plots/tier2_sparsity_scatter.pdf")

    # ── 8. Training time bar plot ─────────────────────────────────────────────
    print("Generating training time plot ...")
    fig = training_time_barplot(df_display.to_dict("records"))
    fig.savefig(PLOTS_DIR / "tier2_training_time.pdf", bbox_inches="tight")
    fig.savefig(PLOTS_DIR / "tier2_training_time.png", bbox_inches="tight", dpi=150)
    plt.close(fig)
    print(f"  Saved plots/tier2_training_time.pdf")
    ...
    summary.to_csv(TABLES_DIR / "tier2_summary.csv")
    print(f"  Saved tables/tier2_summary.csv")
...
def main() -> None:
    ...
    parser.add_argument("--results", type=Path,
                        default=ROOT / "results" / "tier2_n5000_combined.json")
