#!/usr/bin/env python3
"""Tune and run OriginalADMM (OriginalADMMNesterovLSSVM + Elastic Net) on all benchmarks.

Appends results to existing JSON files — never overwrites them.

Phases
------
  1. Tune on Tier 1 (9 datasets)      → best_params_original_admm.json
  ...
  5. Tune on 5f datasets              → best_params_original_admm.json
  6. 5f retuned                       → synthetic_5features_tuned.json
  7. Tune on MK5 datasets             → best_params_original_admm.json
  8. Run MK5 × 30 seeds               → synthetic_mk5.json

Usage
-----
    python scripts/run_original_admm.py [--seeds 30] [--trials 100]
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("results/original_admm.log", mode="a"),
    ],
)
...
RUNNER_NAME  = "OriginalADMMNesterovLSSVM"   # registered name in runner.py
VARIANT_NAME = "OriginalADMM"      # name stored in results JSON
...
PARAMS_FILE = Path("results/tuning/best_params_original_admm.json")
...
def _tune_datasets(datasets: list[str], n_trials: int, folds: int, seed: int) -> None:
    """Tune OriginalADMM on each dataset; append to PARAMS_FILE."""
    import optuna
    ...
    for dataset in datasets:
        ...
        def objective(trial):
            ...
            model = OriginalADMMNesterovLSSVM(
                sigma=sigma, tau=tau, lambda_=lambda_,
                rho=rho, max_iter=500,
            )
...
def main() -> None:
    ...
    log.info("=== OriginalADMM: full benchmark ===")
    ...
    log.info("=== OriginalADMM: all phases complete ===")
