#!/usr/bin/env python3
"""Tune and run FISTANesterov (FISTANesterovLSSVM + Elastic Net) on all benchmarks.

Appends results to existing JSON files — never overwrites them.

Phases
------
  1. Tune on Tier 1 (9 datasets)      → best_params_fista.json
  ...
  5. Tune on 5f datasets              → best_params_fista.json
  6. 5f retuned                       → synthetic_5features_tuned.json
  7. Tune on MK5 datasets             → best_params_fista.json
  8. Run MK5 × 30 seeds               → synthetic_mk5.json

Usage
-----
    python scripts/run_fista_lssvm.py [--seeds 30] [--trials 100]
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("results/fista_lssvm.log", mode="a"),
    ],
)
...
RUNNER_NAME  = "FISTANesterovLSSVM"   # registered name in runner.py
VARIANT_NAME = "FISTANesterov"      # name stored in results JSON
...
PARAMS_FILE = Path("results/tuning/best_params_fista.json")
...
def _tune_datasets(datasets: list[str], n_trials: int, folds: int, seed: int) -> None:
    """Tune FISTANesterov on each dataset; append to PARAMS_FILE."""
    import optuna
...
def main() -> None:
    ...
    log.info("=== FISTANesterov: full benchmark ===")
    ...
    log.info("=== FISTANesterov: all phases complete ===")
