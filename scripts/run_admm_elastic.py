#!/usr/bin/env python3
"""Tune and run ADMMElasticNet (ADMMNesterovLSSVM + Elastic Net) on all benchmarks.

Appends results to existing JSON files — never overwrites them.

Phases
------
  1. Tune on Tier 1 (9 datasets)      → best_params_admm_elastic.json
  ...
  5. Tune on 5f datasets              → best_params_admm_elastic.json
  6. 5f retuned                       → synthetic_5features_tuned.json
  7. Tune on MK5 datasets             → best_params_admm_elastic.json
  8. Run MK5 × 30 seeds               → synthetic_mk5.json

Usage
-----
    python scripts/run_admm_elastic.py [--seeds 30] [--trials 100]
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("results/admm_elastic.log", mode="a"),
    ],
)
...
RUNNER_NAME  = "ADMMNesterovLSSVM"   # registered name in runner.py
VARIANT_NAME = "ADMMElasticNet"      # name stored in results JSON
...
PARAMS_FILE = Path("results/tuning/best_params_admm_elastic.json")

DEFAULT_PARAMS = {
    "sigma": 1.0, "tau": 1.0, "lambda_": 0.1,
    "lambda2_": 0.01, "rho": 1.0,
}
...
def _tune_datasets(datasets: list[str], n_trials: int, folds: int, seed: int) -> None:
    """Tune ADMMElasticNet on each dataset; append to PARAMS_FILE."""
    import optuna
...
def main() -> None:
    ...
    log.info("=== ADMMElasticNet: full benchmark ===")
    ...
    log.info("=== ADMMElasticNet: all phases complete ===")
